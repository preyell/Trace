...
<div class="d-flex justify-content-between align-items-center mb-3">
	...
	<a class="btn btn-primary btn-sm"
		href="${pageContext.request.contextPath}/admin/customers/new"> New
		Customer </a>
</div>
...
<div class="mb-3">
	<div class="input-group">
		...
		<select id="searchSize" name="size" class="custom-select"
			style="max-width: 110px;">
			<option value="10"
				<c:if test="${param.size == '10' || empty param.size}">selected</c:if>>10</option>
			...
			waitForJQ(function($) {
				...
				function load(page) {
					...
					if (xhr)
						xhr.abort();
					$wrap.css('opacity', .6);
					xhr = $.get(ctx + '/admin/customers/table?' + params).done(
							function(html) {
						...
						console.error('Live search failed', jq.status);
					}).always(function() {
						$wrap.css('opacity', 1);
					});
				}
