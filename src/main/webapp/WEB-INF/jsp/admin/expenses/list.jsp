...
<div class="d-flex justify-content-between align-items-center mb-3">
  <h5 class="mb-0"><i class="fa fa-tags mr-2"></i> Additional Expense Labels</h5>
  <a class="btn btn-primary btn-sm" href="${pageContext.request.contextPath}/admin/expenses/new">
    New Label
  </a>
</div>

<c:if test="${not empty message}"><div class="alert alert-primary py-2">${message}</div></c:if>
<c:if test="${not empty error}"><div class="alert alert-danger py-2">${error}</div></c:if>

<div class="mb-3">
  <div class="input-group">
    <input id="searchQ" name="q" class="form-control" value="<c:out value='${param.q}'/>" placeholder="Search name or description"/>
...
(function(w, d){
  ...
  onReady(function(){
    waitForJQ(function($){
      ...
      function load(page){
        var params = $.param({ q: $q.val() || '', size: $size.val() || 10, page: (typeof page === 'number' ? page : 0) });
        if (xhr) xhr.abort();
        $wrap.css('opacity', .6);
        xhr = $.get(ctx + '/admin/expenses/table?' + params)
          .done(function(html){
            $wrap.html(stripScripts(html));
            if (history.replaceState) history.replaceState(null, '', ctx + '/admin/expenses?' + params);
          })
