...
<c:choose>
  <c:when test="${page.totalElements == 0}">
    <div class="p-3 text-muted">No labels found.</div>
  </c:when>
  <c:otherwise>
    ...
    <table class="table mb-0">
      ...
      <tbody>
      <c:forEach var="l" items="${page.content}" varStatus="st">
        <tr>
          <td>${st.index + 1 + (page.number * page.size)}</td>
          <td>${l.name}</td>
          <td><c:out value="${l.description}"/></td>
          <td><span class="badge ${l.system ? 'badge-secondary' : 'badge-light'}">${l.system ? 'Yes' : 'No'}</span></td>
          <td><span class="badge ${l.active ? 'bg-primary' : 'bg-secondary'}">${l.active ? 'Yes' : 'No'}</span></td>
          <td>
            <a class="btn btn-sm btn-outline-primary" href="${pageContext.request.contextPath}/admin/expenses/${l.id}/edit">Edit</a>
            <form method="post" class="d-inline" action="${pageContext.request.contextPath}/admin/expenses/${l.id}/delete" onsubmit="return confirm('Delete this label?');">
              <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
              <button class="btn btn-sm btn-outline-danger" type="submit" ${l.system ? 'disabled' : ''}>Delete</button>
            </form>
          </td>
        </tr>
      </c:forEach>
    ...
    <nav class="px-3 pb-3 pt-2">
      <ul class="pagination pagination-sm mb-0">
        <li class="page-item ${page.first ? 'disabled' : ''}">
          <c:url var="prevUrl" value="/admin/expenses">
        ...
        <c:forEach var="i" begin="${begin}" end="${endIdx}">
          <c:url var="pUrl" value="/admin/expenses">
            <c:param name="q" value="${empty param.q ? '' : param.q}" />
            <c:param name="size" value="${empty param.size ? 10 : param.size}" />
        ...
        <c:url var="nextUrl" value="/admin/expenses">
          <c:param name="q" value="${empty param.q ? '' : param.q}" />
          <c:param name="size" value="${empty param.size ? 10 : param.size}" />
          <c:param name="page" value="${current + 1}" />
        </c:url>
